...
def procesar_subdataset(folder, label):
    ...
    if folder == 'fake_followers':
        columnas_seleccionadas = [1, 2,4, 6, 9, -1]
    else:
        columnas_seleccionadas = [0, 1,3, 5, 8, -3]

    # Seleccionamos esas columnas
    tweets = tweets.iloc[:, columnas_seleccionadas]
    tweets.columns = ['id', 'text', 'user_id', 'in_reply_to_status_id', 'retweeted_status_id','date']
    ...
    stats = merged.groupby('original_tweet_id').agg(
        ...
        usuarios_interactuando=('user_id',list),
        textos_interactuando=('text',list),  # unificamos y quitamos duplicados
...
global_stats = all_stats.groupby('original_tweet_id').agg(
    ...
    textos_interactuando=('textos_interactuando', lambda lists: 
        list({uid for lst in lists for uid in lst})  # unificamos y quitamos duplicados
    ),
...
global_stats.to_csv('resultados/global/interacciones_por_tweet_global_texto.csv', index=False)
global_roots.to_csv('resultados/global/tweets_raiz_global_texto.csv', index=False)
